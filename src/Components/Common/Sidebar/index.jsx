...
class Sidebar extends React.Component {
  ...
  getCollapseInitialState(routes) {
    for (let i = 0; i < routes.length; i++) {
      if (routes[i].collapse && this.getCollapseInitialState(routes[i].views)) {
        return true;
      }else if (this.props.activeRouteName === routes[i].path){
        return true;
      }
    }
    return false;
      ...
      return (
        <li className={this.activeRoute(prop.path)} key={key}>
          <NavLink
            to={prop.path}
            activeClassName=""
            onClick={()=>this.setActiveRouteLink(prop.path)}
          >
            {prop.icon !== undefined ? (
              <>
                <i className={prop.icon} />
  ...
  activeRoute = (routeName) => {
    return this.props.activeRouteName === routeName ? "active" : "";
  };

  setActiveRouteLink(routeName){
